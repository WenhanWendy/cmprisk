...
fall<-summary(mydata1$status)[4] #the actual overall fall rate
fall
censoring<-1-summary(mydata1$status)[4] #the actual overall censoring rate, which is the combination of drop-off and cut-off rate
censoring
dropoff<-1-summary(mydata1$status1)[4] #the drop-off rate
dropoff
cutoff<-1-summary(mydata1$status2)[4] #the administrative censoring rate
cutoff

##### check denpendence #####

sample1<-mydata1.frame(matrix(,clstr,4))
sample2<-mydata1.frame(matrix(,clstr,4))
for (jj in 1:clstr) {
  seq1<-sample(mydata1[mydata1$cluster==jj,5],2,replace=F) #sample from time_t
  sample1[jj,]<-c(seq1,jj,mean(mydata1[mydata1$cluster==jj,2]))
  seq2<-sample(mydata1[mydata1$cluster==jj,7],2,replace=F) #sample from time_o
  sample2[jj,]<-c(seq2,jj,mean(mydata1[mydata1$cluster==jj,2]))
}  
...
model2<-frailtyPenal(s~as.factor(mydata1$x)+cluster(mydata1$cluster),n.knots = 7,kappa = 10000, mydata1=mydata1, RandDist = "Gamma",maxit=50)
